...
| 文档 | 用途 |
| --- | --- |
| [包复用生态](https://michengliang.github.io/seekmoon/books/08-package-reuse-ecosystem/book.html) | 解释 Package Discovery、Package Management、生命周期、证据、评价尺度和治理边界的通用背景。 |
| [SeekMoon：MoonBit 包发现工作台](https://michengliang.github.io/seekmoon/books/09-seekmoon-cli-discovery-workbench/book.html) | 定义 SeekMoon 消费者侧 CLI 工作流、证据模型、命令表面和输出契约的维护版规格。 |

## 项目位置

SeekMoon 位于 MoonBit 包生态事实与消费者依赖采纳判断之间。它不替代 `moon`、Mooncakes 或 registry。它读取 Mooncakes 数据、本地 MoonBit 工具链状态、repository 信号和项目记录，并把这些证据投影为可复查的 CLI 调查工作流。上游事实与本地派生证据保持分离，missing、unknown 和 unverified 等状态会被显式保留。
...
| `docs/` | 研究笔记、原始调查材料和验证报告。正式维护的文档入口是已发布书架，尤其是 08 号书和 09 号书。 |
