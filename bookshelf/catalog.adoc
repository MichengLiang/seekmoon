...
本书架包含结构化写作规范、包复用生态模型和 MoonBit 包发现工具书。07 号书定义书写结构规则；08 号书定义通用包复用生态；09 号书定义 SeekMoon 的 MoonBit 包发现工作台。
...
|实现 SeekMoon CLI
|xref:books/09-seekmoon-cli-discovery-workbench/book.adoc#public-http-sources[Public HTTP Sources]、xref:books/09-seekmoon-cli-discovery-workbench/book.adoc#evidence-state-values[Evidence State Values]、xref:books/09-seekmoon-cli-discovery-workbench/book.adoc#json-output[JSON Output]
|证据来源、状态词、canonical 数据模型、命令契约和输出契约。
|===
