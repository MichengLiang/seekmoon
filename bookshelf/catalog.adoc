...
|实现 SeekMoon CLI
|xref:books/09-seekmoon-cli-discovery-workbench/book.adoc#go-architecture-premises[Architecture Premises]、xref:books/09-seekmoon-cli-discovery-workbench/book.adoc#go-module-layout[Go Module Layout]、xref:books/09-seekmoon-cli-discovery-workbench/book.adoc#appendix-go-implementation-dependencies[附录 F：Go 实现依赖图]
|Go module layout、package 边界、依赖方向、运行时组合、数据流、输出管线和依赖选择。
|===
