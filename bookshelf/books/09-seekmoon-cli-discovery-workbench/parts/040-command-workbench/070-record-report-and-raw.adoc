[#record-command.command]
== `seekmoon record`

`record` 保存采纳判断。它依赖 xref:adoption-record-model[AdoptionRecord, rel=depends-on]。

[source,bash]
----
seekmoon record 1 --conclusion continue-verification --note "Need GFM coverage check."
----

`record` 写入候选、版本、snapshot、evidence refs、conclusion、note 和 not-confirmed items。Conclusion 使用稳定枚举，见 xref:adoption-record-model[AdoptionRecord, rel=depends-on]。

[source,text]
----
recorded  mizchi/markdown@0.6.2
status    continue-verification
file      .seekmoon/records/mizchi-markdown-0.6.2.json
----

[#report-command.command]
== `seekmoon report`

`report` 输出调查报告。它依赖 xref:report-model[Report, rel=depends-on]。

[source,bash]
----
seekmoon report --format markdown
seekmoon report --format json
----

Report 只列实际使用的来源。没有运行 probe，就不写 local validation 结果。没有请求 repository enrichment，就不写 repository activity。

[source,text]
----
report  .seekmoon/reports/markdown-2026-06-21.md
----

[#raw-command.command]
== `seekmoon raw`

`raw` 暴露原始 payload。它服务 audit 和 implementation debugging，不属于普通 discovery path。

[source,bash]
----
seekmoon raw modules
seekmoon raw manifest mizchi/markdown
seekmoon raw module-index mizchi/markdown@0.6.2
seekmoon raw package-data mizchi/markdown@0.6.2 src/api
seekmoon raw skills
----

Raw output 保留上游字段名，不规范化字段，不隐藏原始 shape。Raw output 可以附带 source URL、fetch status 和 timestamp。
