...
`resource.json` 的状态进入 `view`、`api` 或 report 时必须带 source label。`package_data.json`、module index、published source 和 `moon ide doc` 仍可提供 API 与源码证据。
