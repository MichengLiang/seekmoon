...
|Module index、package data、local toolchain evidence when requested by output mode。
|API profile。
...
`--jq <expr>` 需要本机 `jq` 可执行文件。SeekMoon 把命令 JSON output 写入 jq stdin，并返回 jq stdout。
