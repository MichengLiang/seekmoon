...
Declaration 是项目通过 xref:manifest-registry-index-model[Manifest, rel=depends-on] 声明身份、版本、依赖、许可证、平台、入口、构建后端、特性开关或目标环境的阶段。输入是 Authoring 阶段形成的项目材料。输出是机器可读取的声明表面。
...
Publication 是作者把源码包、二进制构件或索引记录发布到 xref:manifest-registry-index-model[Registry、Repository 或 VCS-backed index, rel=depends-on] 的阶段。输入是 Manifest、xref:package-artifact-model[Artifact, rel=depends-on]、源码仓库、发布身份和注册表规则。输出是可被消费者或工具链获取的版本状态。
