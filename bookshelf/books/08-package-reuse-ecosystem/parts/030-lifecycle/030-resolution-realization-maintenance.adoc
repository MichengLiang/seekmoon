...
Resolution 是 Package Manager 根据消费者项目声明、目标平台、版本约束、feature、extras、target framework、lockfile 和 Registry metadata 求出一致依赖图的阶段。输入是 xref:manifest-registry-index-model[Manifest 与 Registry metadata, rel=depends-on]、既有 xref:resolver-lockfile-realization-model[Lockfile, rel=depends-on]、目标环境、索引数据和命令参数。输出是一组具体包版本、变体和依赖边。
...
Realization 是依赖解变成本地实际构件的阶段。输入是 Resolution 输出、xref:package-artifact-model[Artifact, rel=depends-on] 来源、checksum、cache/store、构建工具、目标平台和文件系统。输出是项目可使用的依赖构件、本地安装状态、构建产物或 loader，并可能更新 Lockfile。
