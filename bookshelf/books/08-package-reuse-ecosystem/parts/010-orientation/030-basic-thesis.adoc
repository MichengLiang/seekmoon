...
因此，xref:part-evaluation-scale[评价尺度, rel=depends-on] 不输出无条件总榜。xref:package-level-evaluation[包级评价, rel=depends-on] 回答“某个包在给定查询和消费者上下文中是否适合引入”。xref:ecosystem-level-evaluation[生态级评价, rel=depends-on] 回答“某个生态在给定生命周期阶段是否提供足够机制”。二者使用 xref:part-object-model[同一对象模型, rel=depends-on]，但评价对象不同。
