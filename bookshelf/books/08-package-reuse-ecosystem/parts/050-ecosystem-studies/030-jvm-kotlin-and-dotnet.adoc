...
.NET 的兼容性核心变量是 target framework。一个 NuGet package 是否兼容当前项目，取决于包内 lib/ref/runtimes assets、dependency groups、TFM 和 RID 等规则。NuGet audit 在 restore 阶段暴露漏洞信息，使安全判断进入包管理链路；.NET 8 SDK 起 `dotnet restore` 默认产生安全漏洞警告，.NET 10 又把默认审计范围扩展到传递包。参见 xref:dotnet-restore-audit[.NET restore audit] 与 xref:dotnet-10-transitive-audit[.NET 10 transitive audit]。
