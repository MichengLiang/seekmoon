...
vcpkg manifest mode 使用 `vcpkg.json` 声明 dependencies、features、default-features、builtin-baseline、overrides、supports、version 和 port-version。vcpkg registries 提供 port 和 version database；triplet 捕捉 target environment，例如 CPU、OS、compiler、runtime 和 linkage。Binary caching 影响同一解析结果是否需要本地构建。参见 xref:vcpkg-manifest[vcpkg manifest mode]、xref:vcpkg-json[vcpkg.json reference] 与 xref:vcpkg-triplets[vcpkg triplets]。
