...
deps.dev / Open Source Insights 提供跨 npm、Maven、PyPI、Cargo、NuGet、Go、RubyGems 等生态的包、版本、依赖图、许可证、项目映射和 OSV advisory 数据。它适合作为图谱和 advisory 聚合层，不是各注册表的写入事实源。参见 xref:deps-dev-docs[deps.dev Docs]。

Libraries.io 覆盖多个 package manager 和源码仓库，提供 package metadata、dependencies、dependents、dependent repositories、contributors、SourceRank、stars 和 forks 等字段。其公开说明标注数据来自互联网抓取且不保证准确，因此应作为发现提示和人工复核入口。参见 xref:libraries-io-api[Libraries.io API] 与 xref:libraries-io-data[Libraries.io data]。

ecosyste.ms 提供 packages、resolve、archives、licenses、repos、advisories、SBOM 等开放 API 族。它适合作为开放数据基础设施，但每个端点的字段和覆盖范围需要单独审查。参见 xref:ecosystems-packages[ecosyste.ms packages]。
