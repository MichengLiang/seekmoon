...
包级评价回答一个问题：某个候选包在给定查询和消费者上下文中是否适合引入。它不回答世界上哪个包绝对最好。
...
相关性证据可以来自 description、README、keywords、classifiers、API symbols、examples、documentation headings、package categories 和 ecosystem-specific tags。一个高质量 JSON parser 不应因为下载量高而出现在 OAuth client 查询前列。

=== 质量与信任画像

进入主排序后，包级评价可以组合文档、依赖图、维护、兼容、安全、采用度和可复现信号。
...
这个表达式表示需要保留多个解释分量。具体实现可以使用加权、门控、分段函数或规则系统，但输出必须说明排序原因。
