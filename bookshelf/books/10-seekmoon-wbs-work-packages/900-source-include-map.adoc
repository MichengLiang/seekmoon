[#source-include-map.reference]
== 来源引用图

本章记录 10 号书引用 9 号书的位置。它服务维护者复查 include 路径、行号范围和交叉引用质量。
...
|Public HTTP Sources、Source Reader Boundary、`sync` and `search` data flow。
...
|Output contracts and Output Pipeline。
...
当 9 号书章节发生行号变化时，维护者必须检查本书所有 `lines=` include。整章 include 使用 9 号书章节锚点；局部 include 使用行号范围。局部 include 不应包含 9 号书原始 anchor 行，避免同一本渲染书中出现重复 id。
