[#wp02-canonical-model-and-contracts.work-package]
...
|`Report` sections and report input structures。
...
* Evidence wrapper JSON round-trip。
* State enum string parsing and validation。
...
* Fuzz tests for coordinate and relpath derivations。
...
WP02 完成后，下游工作包可以依赖稳定对象语言实现来源读取、服务组合和输出投影。
